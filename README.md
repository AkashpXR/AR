...
3. Check the printed `MAT` / `PLAN` lines and `docs/models/<id>/poster.webp` (or open `docs/qa.html?m=<id>` on the local preview), then `git add -A && git commit -m "Add garment <id>" && git push`.
...
- `tools/glb_to_web.py` (called by `make_garment.py`) turns a CLO `.glb` into `model.glb` (Draco) and `poster.webp`. Every export differs (mannequins from 95k to 2.4M faces, hundreds of topstitch meshes, hair as alpha cards or solid geometry), so geometry is reduced against a global budget of 250k triangles (`--budget`): hair cards and tiny parts (eyes, lashes) are kept intact, the mannequin body is capped at 100k, all topstitch trims together at 40k (1.5k each), flat dense artefacts (shoe soles) collapse to 2k, and the cloth gets the rest (at most 35% of the original). Hair cards (alpha-textured materials in the head region) become a matte double-sided alpha cutout at 0.3 with hard-alpha textures; solid hair exported as "blend" is made opaque. Opaque PNG textures are re-saved as JPEG. Read the printed `MAT ...` classification and `PLAN ...` lines for each new garment; `--hair Name1,Name2` forces materials to be treated as hair cards.
