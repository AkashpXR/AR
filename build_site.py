...
<script type="module">
  ...
  // Android test switch: ?ar=scene-viewer prefers Google's native Scene Viewer over in-page WebXR
  const arPref = new URLSearchParams(location.search).get('ar');
  if (arPref === 'scene-viewer') mv.setAttribute('ar-modes', 'scene-viewer webxr quick-look');
  if (arPref === 'webxr') mv.setAttribute('ar-modes', 'webxr quick-look');
